...
make migrate       # применить миграции (000001..000010)
...
# seed-pattern-content и seed-pattern-cards не имеют make-таргетов —
# запускаются через docker compose (см. ниже):
docker compose run --rm seed-pattern-content  # контент паттернов (seeds/pattern_content.yaml)
docker compose run --rm seed-pattern-cards    # карточки паттернов (seeds/pattern_cards.yaml)
```
...
- `docker compose run --rm seed-pattern-content`, `docker compose run --rm seed-pattern-cards`
  завершаются без ошибок; в БД появился контент паттернов (techniques/symptoms/checklist)
  и карточки паттернов.
