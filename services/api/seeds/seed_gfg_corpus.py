#!/usr/bin/env python3
"""Seed the Realgo GeeksforGeeks corpus: dataset-imported problems and their
primary problem<->subpattern links.

Same shape as seed_atlas_corpus.py (LeetCode): problems are deduplicated
against existing rows by (platform_id, external_slug); link upserts never
touch curated rows (tier IS NOT NULL) — only corpus-created links (tier IS
NULL) refresh their position.

Usage:
  seed_gfg_corpus.py atlas_gfg_problems.yaml
  seed_gfg_corpus.py --validate-only atlas_gfg_problems.yaml
"""
import argparse
import os
import sys
...
def load_corpus(path):
    ...
    if data.get("platform") != "geeksforgeeks":
        raise ValueError(f"{path}: expected platform: geeksforgeeks")
    ...
    for entry in subpatterns:
        ...
        for position, item in enumerate(problems, 1):
            slug = require(item, "slug", f"corpus {code}")
...
def parse_args(argv):
    ...
    parser.add_argument("corpus", help="GeeksforGeeks corpus YAML")
...
def main(argv=None):
    ...
    print(f"seeded gfg corpus: {len(rows)} problems ({inserted} upserted), {linked} links written")
