...
## Community Company Problems Dataset (пониженное доверие)

Слой `source_type = community_dataset` — пониженное доверие, ниже
`dataset`/`manual`/`community` во всех отношениях (см. миграцию
`000026_add_community_dataset_source_type`), но выше отсутствия данных
вообще. Не переиспользует `'community'`, потому что `community`-строки
навсегда защищены от перезаписи в `seed_company_problems.py`/
`seed_gfg_company_problems.py` — низкодоверенная запись заблокировала бы
будущее качественное evidence по той же паре. Не пишет в
`subpattern_companies` вообще — relevance/confidence Company Overlay
остаются производными только от `source_type = 'dataset'`.

Слой собирается из НЕСКОЛЬКИХ независимых источников, каждый со своим
build-скриптом, но одним общим сидером:

```sh
DATABASE_URL='postgres://postgres:postgres@localhost:5432/freeburger?sslmode=disable' \
  python seed_community_company_problems.py \
    atlas_community_company_problems.csv.gz \
    atlas_researched_company_tasks.csv.gz
```

Все CSV передаются в сидер ОДНИМ вызовом — не по одному. Ребилд слоя
скоупится по platform_id, а не по исходному файлу, поэтому одиночный
вызов с CSV только одного источника удалит evidence всех остальных
источников на этой платформе при следующем ресиде.

### Источник 1: dr-o-ne (LeetCode, 10 компаний, 29 пар)

`atlas_community_company_problems.csv.gz` — компании, у которых 0 строк в
обоих датасетах выше: Anthropic, Block (алиас на существующую "Square" —
см. ниже), Canva, Chime, Epic Games, Etsy, Fortinet, Peloton, Plaid,
Zendesk. Источник — единственный скрейпер,
github.com/dr-o-ne/leetcode-company-problem-frequency (README не
раскрывает происхождение цифр; колонки `Times`/`Period` совпадают по
форме с платной company-tag фичей LeetCode, а не с текстом
интервью-репортов кандидатов, как у четырёх источников
`build_company_problems.py`).

`"Block"` в CSV аккуратно алиасится на company_code `cmp_square`: Square
Inc. в 2021 переименовалась в Block Inc., и "Square" уже существует в
основном LeetCode-датасете (35 строк). Без алиаса появился бы отдельный
`cmp_block`, раздваивающий evidence одной компании — см. комментарий в
`build_community_company_problems.py`. Побочный эффект: после сидирования
display-имя компании в БД сменится с "Square" на "Block" (ON CONFLICT
берёт имя из последнего запуска).

Пересборка из свежего клона источника:

```sh
git clone https://github.com/dr-o-ne/leetcode-company-problem-frequency
python build_community_company_problems.py \
  --repo leetcode-company-problem-frequency \
  --out atlas_community_company_problems.csv.gz
```

`COMPANY_FILES` в билдере — явный allow-list, не "весь репозиторий":
расширять его на остальные ~550 компаний репозитория — сознательное
решение (вопрос происхождения данных снова открывается), не побочный
эффект перезапуска скрипта.

### Источник 2: ручной ресерч по HackerRank OA (LeetCode+HackerRank, 37 компаний, 61 пара)

`atlas_researched_company_tasks.csv.gz` — из
`hackerrank_company_task_dataset_2022_2026_v7.xlsx` (лист `Pairs_2022plus`,
249 строк, методология описана на листе `Methodology`). В отличие от
dr-o-ne это НЕ скрейп: каждая строка — отдельная процитированная находка
(interview-experience пост, Blind/Glassdoor и т.п.) с `interview_year`,
`confidence` (High/Medium) и `evidence_type` (Direct/Cross-source). Тем не
менее это единственный (не перепроверенный независимо) ресерч-проход, а не
несколько независимо поддерживаемых репо, поэтому остаётся в
`community_dataset`, а не в `dataset`.

Импортируются только строки с `public_equivalent_url` на уже
отслеживаемой платформе (leetcode.com/problems/, geeksforgeeks.org/problems/,
hackerrank.com/challenges/) — 61 из 249. Остальные 188 пропускаются:
159 — proprietary-задачи компании без публичного эквивалента (только
текстовое описание, например "Mango"/"Keypad Fun" у Cisco), 26 — ссылки на
geeksforgeeks.org/dsa/... (это статьи-объяснения алгоритма, не страницы
практикуемой задачи geeksforgeeks.org/problems/..., куда ведут все ссылки
в основном GFG-датасете) плюс 1 на cses.fi (платформа, которую мы вообще
не отслеживаем). `--report-skipped` печатает все пропуски с причиной.

`evidence_count` кодирует confidence/evidence_type, для которых в схеме
нет отдельных полей: 1 базово, +1 за `confidence=High`, +1 за
`evidence_type=Cross-source` (1..3). `last_seen` — 1 июля года из
`interview_year` (в датасете есть только год, не дата).

Два алиаса поверх обычной схемы (см. докстринг билдера):
`"Twitter (X)"` → `cmp_twitter` (существующие 111 строк перевешивают `X`
с 51 — то же правило, что для Block/Square: сливать в код с большим
существующим evidence) и `"JPMorgan Chase"` → `cmp_jpmorgan` (91 против
79 у "J.P. Morgan"). Кстати, "J.P. Morgan"/"JPMorgan" — это САМИ по себе
нерешённый сплит в primary-датасете (тот же класс бага, что Twitter/X,
Snap/Snapchat, Walmart/Walmart Labs, SAP/SAP Labs) — не чиню здесь, нужен
ребилд `atlas_company_problems.csv.gz` из четырёх внешних клонов.

Пересборка (xlsx лежит вне репозитория, путь передаётся явно):

```sh
python build_researched_company_tasks.py \
  --xlsx /path/to/hackerrank_company_task_dataset_2022_2026_v7.xlsx \
  --out atlas_researched_company_tasks.csv.gz
```

### Общее

Идемпотентен. Запускать ПОСЛЕ seed_atlas.py/seed_atlas_corpus.py (нужны
линки задач на подпаттерны, хотя этот сидер их не использует — оставлено
для единообразия с двумя другими company-датасетами) и после миграции
`000026_add_community_dataset_source_type`. Порядок относительно
`seed_company_problems.py`/`seed_gfg_company_problems.py` не важен: при
любой последовательности primary-датасеты перезапишут `community_dataset`
для компании, если та когда-нибудь появится в основных источниках.
