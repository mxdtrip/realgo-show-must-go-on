...
SELECT
    ...
    p.title,
    p.url,
    CASE WHEN pl.code IN ('leetcode','hackerrank','codeforces','geeksforgeeks') THEN pl.code ELSE 'custom' END AS platform,
...
VALUES (sqlc.arg(user_id)::bigint, sqlc.arg(problem_id)::bigint, NOW(), 0, 2.5, 0, 0, 0, 'fsrs', 0)
...
WITH scoped_problems AS (
    SELECT
        ...
        p.url,
        CASE
            WHEN pl.code IN ('leetcode', 'hackerrank', 'codeforces', 'geeksforgeeks') THEN pl.code
            ELSE 'custom'
        END AS platform,
        CASE
